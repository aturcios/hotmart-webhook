...
const WEEK_TASKS = [
  // WEEK 1: Setup & Lanzamiento
  {
    ...
    unique: [
      ...
      { day: 2, member: 0, task: "Crear el primer video VSL adaptado con IA para TikTok/Reels (link recomendado del productor)" },
      ...
      { day: 6, member: 0, task: "Grabar video de testimonio/review del producto (sin haberlo comprado: usar materiales del productor)" },
      { day: 7, member: 2, task: "Reportar resultados de la semana: grupos donde se compartió, reacciones recibidas" }
    ]
  },
  // WEEK 2: Contenido & Engagement
  {
    ...
    unique: [
      ...
      { day: 10, member: 3, task: "Crear TikTok/Reel de 30 seg usando materiales del productor (plantilla provista)" },
      ...
      { day: 13, member: 0, task: "Crear un hilo de Twitter/X o post largo en Facebook sobre mecánica básica de motos" },
  ...
  // WEEK 3: Escalado
  {
    ...
    unique: [
      { day: 15, member: 0, task: "Lanzar una campaña de '7 días aprendiendo mecánica' en TikTok/Reels (serie de videos)" },
      ...
      { day: 20, member: 0, task: "Publicar contenido con los testimonios recolectados (link de testimonios del productor)" },
...
export default function App() {
  ...
  const [view, setView] = useState("day"); // day | overview
  ...
  function memberProgress(memberId) {
    ...
    for (let d = 1; d <= 30; d++) {
      ...
      dayTasks.filter(t => t.member === memberId).forEach(t => {
        total++;
      ...
      <div style={{ maxWidth: 480, margin: "2rem auto", padding: "0 1rem" }}>
        <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "2rem" }}>
          ...
          <button onClick={() => { if (startDate) setStarted(true); }}
            style={{ width: "100%", padding: "10px", background: "#185FA5", color: "#fff", border: "none", borderRadius: "var(--border-radius-md)", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
            Iniciar campaña
          </button>
        </div>
      </div>
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "1rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>Ventas:</span>
          <button onClick={() => setSales(s => Math.max(0, s - 1))} style={{ padding: "2px 10px", fontSize: 14 }}>−</button>
          <span style={{ fontWeight: 500, minWidth: 24, textAlign: "center" }}>{sales}</span>
          <button onClick={() => setSales(s => s + 1)} style={{ padding: "2px 10px", fontSize: 14 }}>+</button>
        </div>
      </div>
      ...
      <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "12px 16px", marginBottom: "1rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "var(--color-text-secondary)", marginBottom: 6 }}>
          <span>Progreso general de campaña</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 10, marginBottom: "1rem" }}>
        {[
          ...
          { label: "Tareas hoy", value: tasks.length },
          { label: "Completadas hoy", value: tasks.filter(t => completed[t.id]).length },
          ...
          <div key={c.label} style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "10px 12px" }}>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 22, fontWeight: 500, color: "var(--color-text-primary)" }}>{c.value}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: "1rem" }}>
        {["day", "overview"].map(v => (
          <button key={v} onClick={() => setView(v)}
            style={{ padding: "6px 16px", fontSize: 13, background: view === v ? "#185FA5" : "transparent", color: view === v ? "#fff" : "var(--color-text-secondary)", border: `0.5px solid ${view === v ? "#185FA5" : "var(--color-border-secondary)"}`, borderRadius: "var(--border-radius-md)", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: "1rem" }}>
            ...
            <div style={{ flex: 1, textAlign: "center" }}>
              <span style={{ fontWeight: 500, fontSize: 15 }}>
                Día {currentDay} — {fmt(addDays(start, currentDay - 1))}
              </span>
